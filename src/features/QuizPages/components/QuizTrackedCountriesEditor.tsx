...
interface QuizTrackedCountriesEditorProps {
    trackedCountries: TrackedCountry[]
    defaultCountry: TrackedCountry
    onTrackedCountriesChange: (countries: TrackedCountry[]) => void
}

const QuizTrackedCountriesEditor: React.FC<QuizTrackedCountriesEditorProps> = ({ trackedCountries, defaultCountry, onTrackedCountriesChange }) => {
    const [localTrackedCountries, setLocalTrackedCountries] = useState<TrackedCountry[]>([])
    ...
    useEffect(() => {
        if (trackedCountries && trackedCountries.length > 0) {
            setLocalTrackedCountries(trackedCountries)
        } else {
            setLocalTrackedCountries([defaultCountry])
        }
    }, [trackedCountries, defaultCountry])
    ...
    const handleAddCountry = () => {
        ...
        if (localTrackedCountries.some((c) => c.country === selectedCountry.country)) {
            toast.error("Country already tracked")
            return
        }
        if (localTrackedCountries.length >= 3) {
            toast.error("Maximum of 3 countries allowed")
            return
        }
        const updatedCountries = [...localTrackedCountries, selectedCountry]
        setLocalTrackedCountries(updatedCountries)
        setNewCountry("")
        onTrackedCountriesChange(updatedCountries)
    ...
    const handleConfirmDelete = () => {
        ...
        const updatedCountries = localTrackedCountries.filter((_, i) => i !== deleteCountryIndex)
        setLocalTrackedCountries(updatedCountries)
        setDeleteDialogOpen(false)
        setDeleteCountryIndex(null)
        onTrackedCountriesChange(updatedCountries)
    ...
    return (
        <>
            <Box sx={{ display: "flex", gap: 2, mb: 2 }}>
                ...
                <Button variant="contained" onClick={handleAddCountry} disabled={localTrackedCountries.length >= 3 || !newCountry}>
                    Add
                </Button>
            </Box>

            <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1 }}>
                {localTrackedCountries.map((country, index) => (
                    <Chip
                        key={index}
...
export default QuizTrackedCountriesEditor
