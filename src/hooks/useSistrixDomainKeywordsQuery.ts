...
const fetchSistrixData = async (params: Record<string, string>): Promise<DomainSistrixResponse> => {
    const { data } = await axios.get("/api/sistrix/keyword-domain", { params })
    return data
}

export const useSistrixDomainKeywordsData = (
    domain: string,
    country: string = "de",
    ...
    return useQuery<DomainSistrixResponse, Error, DomainSistrixResponse, unknown[]>({
        queryKey: ["sistrix-domain-keywords", domain || null, country, JSON.stringify(options)],
