...
const CompetitorServicesEditor: React.FC<CompetitorServicesEditorProps> = ({ open, onClose, competitor, generalServices, onSave }) => {
    ...
    const filteredServices = useMemo(() => {
        return generalServices.filter((service) => service.title!.toLowerCase().includes(searchTerm.toLowerCase())).sort((a, b) => a.title!.localeCompare(b.title!))
    ...
    const handleToggleService = (service: GeneralService) => {
        ...
        if (isSelected) {
            ...
            setSelectedServices((prev) => [...prev, { ...service, analysisType: "manual" }])
        }
    }
        ...
        <>
            <Dialog open={open} onClose={onClose} fullWidth maxWidth="md">
                ...
                <DialogContent dividers>
                    <TextField value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} label="Search services" variant="outlined" fullWidth sx={{ mb: 2 }} />
