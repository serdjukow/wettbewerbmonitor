...
const rows = [createData(1, "Cupcake", 305, 3.7, 67, 4.3), createData(2, "Donut", 452, 25.0, 51, 4.9)]
...
// Интерфейс для ответа от API кредитов
...
const DashboardPage = () => {
    ...
    // Задаём тип состояния для creditsData как число, строка или null
    const [creditsData, setCreditsData] = useState<number | string | null>(null)
    ...
    useEffect(() => {
        const fetchCredits = async () => {
            try {
                ...
                if (data.error) {
                    setError(data.error.error_message || "Unknown error")
                } else {
                    setCreditsData(data?.answer?.[0]?.credits?.[0]?.value ?? "No data")
                }
            } catch (err) {
    ...
    // Избегаем скачков верстки при достижении последней страницы с пустыми строками.
